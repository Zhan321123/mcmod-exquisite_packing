...
public class ChainPlantItem extends BlockItem {
    private final Block plantBlock;
    private static final List<Block> canPlaceOn = List.of(
            Blocks.DIRT, Blocks.ROOTED_DIRT, Blocks.PODZOL, Blocks.GRASS_BLOCK, Blocks.COARSE_DIRT, Blocks.MOSS_BLOCK
    );
    private static final List<Block> canReplace = List.of(
            Blocks.AIR, Blocks.WATER, Blocks.SNOW, Blocks.GRASS, Blocks.TALL_GRASS
    );

    public ChainPlantItem(Block block, Block plantBlock) {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> componentList, @NotNull TooltipFlag flag) {
        ...
        componentList.add(new TranslatableComponent(I18n.get(ExquisitePacking.MODID + ".chain_planting.desc")));
    ...
    public @NotNull InteractionResult useOn(@NotNull UseOnContext uoc) {
        ...
        if (canPlaceOn.contains(level.getBlockState(pos).getBlock()) &&
            ...
            if (chainPlant(level, getChainPlantPos(level, pos))) {
                ...
                SoundType soundType = plantBlock.getSoundType(plantBlock.defaultBlockState(), level, pos, player);
    ...
    // get the block positions where can plant
    private List<BlockPos> getChainPlantPos(Level level, BlockPos pos) {
        ...
        for (int i = -length; i <= length; i++) {
            for (int j = -length; j <= length; j++) {
                BlockPos here = pos.offset(i * distance, 0, j * distance);
                BlockPos above = here.above();
                if (canPlaceOn.contains(level.getBlockState(here).getBlock()) &&
                        canReplace.contains(level.getBlockState(above).getBlock())) {
                    posList.add(above);
                    size++;
                }
                if (size == max) {
    ...
    // chain-plant the blocks
    private boolean chainPlant(Level level, List<BlockPos> posList) {
        if (posList.size() == 0) {
            return false;
        }
        for (BlockPos pos : posList) {
            level.setBlock(pos, this.plantBlock.defaultBlockState(), 4);
        }
        return true;
    }
}
